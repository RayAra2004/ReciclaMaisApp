...
public class MaterialPagingSource extends ListenableFuturePagingSource<Integer, Material> {
    UserRepository userRepository;
    Integer initialLoadSize = 0;

    public MaterialPagingSource(UserRepository userRepository){
    ...
    @Nullable
    @Override
    public Integer getRefreshKey(@NonNull PagingState<Integer, Material> pagingState) {
    ...
    @NonNull
    @Override
    public ListenableFuture<LoadResult<Integer, Material>> loadFuture(@NonNull LoadParams<Integer> loadParams) {
        ...
        ListenableFuture<LoadResult<Integer, Material>> lf = service.submit(new Callable<LoadResult<Integer, Material>>() {
            /**
             * Tudo que estiver dentro dessa função será executado na nova linha de execução.
             */
            @Override
            public LoadResult<Integer, Material> call() {
                List<Material> materiaisList = null;
                // envia uma requisição para o servidor web pedindo por uma nova página de dados (bloco de produtos)
                materiaisList = userRepository.loadMateriais(loadParams.getLoadSize(), finalOffSet);
                Integer nextKey = null;
                if(materiaisList.size() >= loadParams.getLoadSize()) {
                    nextKey = finalNextPageNumber + 1;
                }
                // monta uma página do padrão da biblioteca Paging 3.
                return new LoadResult.Page<Integer, Material>(materiaisList,
                        null,
                        nextKey);
            }
        });
